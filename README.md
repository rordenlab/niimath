...
niimath has a few features not provided by fslmaths:
 ...
 - `allineate <base> [opts]`: affine registration to match 'base' (from AFNI 3dAllineate)
   ...
   - **Fast engine (the default):** `-cost fast` (Hellinger/MI, robust cross-modal) is the default cost; `-cost fastcr` (correlation-ratio) is its same-modality variant. Both select an independent, SPM/FLIRT-inspired multiresolution 12-DOF estimator that is typically several times faster; `-cost hel`/`lpc`/`lpa`/`ls` select the ordinary AFNI-style engine instead. It runs a fixed schedule with internal sampling, so it rejects `-warp`/`-interp`/`-source_automask`/`-dark_automask`/`-zoom` (use a normal `-cost` for those) — but the `-com`/`-sym`/`-symd`/`-symb` header seeds DO work with it (they only re-pose the moving header before the fast fit). Default/`-cmass` chooses the supplied-affine or COM-recentered initialization from initial dependence×overlap, `-com` forces COM, and `-nocmass` forces the supplied affine; `-final`/`-master`/`-savemat` are also honored. `-cost` is last-one-wins. `-weight <img>` (fast engine only, **experimental soft-focus**; a base-space region image, 3D with dims and world frame matching `base`) biases the fine-stage fit toward a region — e.g. a brain mask — by remapping the image to `[0.5, 1]` over the whole grid (an AFNI-style whole-head anchor that stops global scale collapsing into the scalp); it is a soft focus, **not an exclusion mask** (a zeroed region is down-weighted to the floor, never removed). Because the floor lifts every foreground voxel, admission needs the ROI to have real contrast above the floor over the `base` foreground — a flat/background-only weight (nothing rising above the floor) is rejected, not merely a "positive" one. When `-weight` is given the default fast engine does not silently fall back to the ordinary engine (which ignores the weight).
