...
1. The command "fslmaths img -add 0 j -odt input" will convert a uint8 image float output despite explicit request to retain input type. This occurs if the input header as a non-unitary scale slope and intercept. In contrast, niimath retains both the datatype and the intensity scaling parameters. 
...
4. The fslmaths `-dilD` funtion does not do what it claims. It introduces a blurring effect that reduces edge artifacts that plague iterative morphology operations. Unfortunately, this effect is conducted in a consistent order that introduces a spatial shift in signal. In contrast, niimath does the dilation as described. Note there are [better solutions](https://github.com/neurolabusc/niiSmooth) for these functions.
