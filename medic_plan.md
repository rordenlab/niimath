...
**Resolved: choose one active level by frame count** — multi-frame runs parallelize the per-frame MCPC/ROMEO loop; a single-frame run leaves that loop serial so ROMEO's inner regions use the team. No nested teams are created. Measured scaling on the 170-frame run is 3.75× from 1→8 threads (manifest §5.3).
