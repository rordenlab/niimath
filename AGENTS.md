...
1. **The C `qsort()` comparator is a per-comparison indirect call (`call_indirect`) — a severe WASM penalty.** emscripten/musl `qsort` (smoothsort) over millions of elements with an un-inlined comparator is ~100× slower than native libc qsort, enough to make the WASM build appear to *hang* (100% CPU, flat RSS). It hung `-deface`/`-allineate` on large images via `al_automask`, which sorted all 16M voxels just to read one percentile. **Fix: never use `qsort`+comparator on large/hot arrays in code that targets WASM** — use a comparator-free O(n) routine. allineate now uses a shared 3-way-quickselect `al_select_rank` for all percentile/median needs (automask, autoweight median, al_quantile, source_automask). **Project-wide audit — other `qsort`+comparator sites with the same latent WASM cost (NOT yet converted; only the allineate path was in scope):** (A) one-shot image-sized percentile: `unifize.c:46` (`-unifize` p98). (B) *tiny `qsort` inside a per-voxel loop* (millions of comparator indirect calls): `unifize.c:191`, `coreFLT.c` `-fmedian` (~2959/2995), `-Tmedian`/`-Tperc` (~2449/4539), `-roc`/`-pval` (~4850). For (A) use quickselect; for (B) replace the per-kernel `qsort` with a comparator-free insertion sort / selection (small N). The two `qsort_floatint` calls left in allineate are on ~46-element candidate lists — negligible.
