...
4. **`int` truncation of voxel-count products (real UB, needs its own pass)** — ~29 sites in `coreFLT.c`/`conform.c` compute `int nvox3D = nim->nx * nim->ny * nim->nz` (and `int nvol`, `int nvoxOut`) even though `nim->nx/ny/nz/nvox` are `int64_t`. `nifti_io` guards total `nvox` only against **int64** overflow (`nifti_io.c:1498/1611`), so an image with >2³¹ voxels reads fine but then truncates these `int` locals. Reachable from a parseable, highly-compressible `.nii.gz`/`.nii.zst` (few MB on disk, decompresses to the full payload) on a big-RAM host: a product that is an exact multiple of 2³² (e.g. `2048³`) truncates `nvox3D` to 0 → `nvox / nvox3D` **div-by-zero (SIGFPE)**; other values (e.g. `1626³`) undersize `conform`'s `in_img` while the reslice loop still runs the true extent → **OOB read**. Many ops incidentally bail via their `if (nvox3D*nVol != nim->nvox) return 1;` guard. **A count+size checked allocator does NOT fix this** — the overflow is upstream of the allocation, in the `int·int` product; the fix is `int64_t`/`size_t` locals + validate `≤ nim->nvox`, ideally a shared `nii_nvox3d(nim, &out)` helper. Pre-existing (the `_mm_malloc` sweep neither caused nor touched it); ~15-29 sites; scope as its own pass with the conformance suite (several sites already use `size_t nvox3D`, so the pattern is inconsistent). Confirmed by the 2026-07-03 round-2 audit.
5. **allineate OOM/robustness (partial):** the `-allineate`/`-deface` coarse-search **all-`AL_BIGVAL` fail-open was fixed** 2026-07-03 (`al_scalar_ransetup` now returns nonzero when `ngood < 1` so `al_register` aborts instead of proceeding on an identity transform — a `-deface` privacy fix; only affects the already-failed path, no successful-registration trajectory changes, so the `-ffast-math` golden match is unaffected). **Still open (OOM-only, low severity):** the BLOK `realloc(elm[dd], …)` at `allineate.c:~914` leaks the old block on failure (fix: realloc into a temp). Note `allineate.c` is shared verbatim with the standalone `allineate/` project — sync fixes there.
