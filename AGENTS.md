...
- `HAVE_GPL` — optional GPL `-spmcoreg`/`-spm_deface` (SPM rigid-body coregistration and defacing); OFF by default so builds stay BSD-2. Sources in the `niimath_gpl` git submodule at `src/GPL`; enable with `GPL=1 make` / `cmake -DENABLE_GPL=ON`. A binary built with it is a GPL-2 combined work (version string ends ` GPL` vs ` BSD`)
