...
/* Affine (12 DOF) image registration
   ...
   Supports Hellinger (default), lpc+ZZ, lpa+ZZ, and Pearson (ls) cost functions
...
/* Final interpolation codes (for output reslicing) */
#define AL_FINAL_NN       0  /* Nearest-neighbor */
#define AL_FINAL_LINEAR   1  /* Trilinear */
#define AL_FINAL_CUBIC    3  /* Tricubic (default, matches AFNI) */
...
typedef struct {
    ...
    int final_interp;      /* AL_FINAL_* code for output reslicing (default: AL_FINAL_CUBIC) */
} al_opts;
...
static inline int al_parse_subopts(int *ac, int argc, char **argv, al_opts *opts,
    ...
    while (*ac + 1 < argc && argv[*ac + 1][0] == '-') {
        ...
        if (!strcmp(argv[*ac], "-cmass")) {
            ...
            opts->final_interp = AL_FINAL_NN;
        } else if (!strncmp(argv[*ac], "-linear", 4) || !strncmp(argv[*ac], "-trilinear", 6)) {
            opts->final_interp = AL_FINAL_LINEAR;
        } else if (!strncmp(argv[*ac], "-cubic", 4) || !strncmp(argv[*ac], "-tricubic", 6)) {
            opts->final_interp = AL_FINAL_CUBIC;
