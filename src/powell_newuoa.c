...
/*---------------------------------------------------------------------------*/
/* Helper: sort doubles and simultaneously reorder an array of pointers.     */
/*---------------------------------------------------------------------------*/

typedef struct { double val; void *ptr; } qds_pair;

static int qds_compare(const void *a, const void *b) {
    double va = ((const qds_pair *)a)->val;
    double vb = ((const qds_pair *)b)->val;
    return (va < vb) ? -1 : (va > vb) ? 1 : 0;
}

static void qsort_doublestuff(int n, double *eldar, void **vpt) {
    int i;
    qds_pair *pairs;
    if (n < 2) return;
    pairs = (qds_pair *)malloc(sizeof(qds_pair) * (size_t)n);
    if (pairs == NULL) return;
    for (i = 0; i < n; i++) { pairs[i].val = eldar[i]; pairs[i].ptr = vpt[i]; }
    qsort(pairs, (size_t)n, sizeof(qds_pair), qds_compare);
    for (i = 0; i < n; i++) { eldar[i] = pairs[i].val; vpt[i] = pairs[i].ptr; }
    free(pairs);
}
...
/* 2D array helper: contiguous block with row pointers */
static double **x01_2d = NULL; static int x01_2d_nrow = 0, x01_2d_ncol = 0;
static void resize_x01_2d(int nrow, int ncol) {
    if (x01_2d_nrow >= nrow && x01_2d_ncol >= ncol) return;
    if (x01_2d != NULL) { if (x01_2d[0] != NULL) free(x01_2d[0]); free(x01_2d); }
    x01_2d = (double **)malloc(sizeof(double *) * (size_t)nrow);
    if (x01_2d == NULL) { x01_2d_nrow = 0; x01_2d_ncol = 0; return; }
    x01_2d[0] = (double *)calloc((size_t)(nrow * ncol), sizeof(double));
    if (x01_2d[0] == NULL) { free(x01_2d); x01_2d = NULL; x01_2d_nrow = 0; x01_2d_ncol = 0; return; }
    for (int i = 1; i < nrow; i++) x01_2d[i] = x01_2d[0] + i * ncol;
    x01_2d_nrow = nrow; x01_2d_ncol = ncol;
}
                               ...
                               int maxcall , double (*ufunc)(int,double *) )
{
   ...
   double vbest ;
   static double *w = NULL ; static int w_len = 0 ;
   static double *x01val = NULL ; static int x01val_len = 0 ;
   ...
   resize_x01_2d(nx01, ndim) ;
   do { if(x01val_len < (nx01)) { free(x01val); x01val = (double*)calloc((size_t)(nx01),sizeof(double)); x01val_len = (nx01); } } while(0) ;
   for( tt=0 ; tt < nx01 ; tt++ ) x01val[tt] = BIGVAL; /* mark as unready */

   /* copy x[] into x01_2d[0], scaling to 0..1 range */
...
#define CALFUN_ERROR(xxx)                                      \
 do{ if( calfun_err ){                                         \
       ...
       for( aa=0 ; aa < ndim ; aa++ )                          \
         fprintf(stderr," %g",sxmin_arr[aa]+xxx[aa]*sxsiz_arr[aa]) ;   \
       fprintf(stderr,"\n") ;                                  \
   } } while(0)

   if( verb )
     fprintf(stderr,"++ Powell: evaluation at initial guess\n") ;
   for( ii=0 ; ii < ndim ; ii++ ){
     x01_2d[0][ii] = (x[ii] - sxmin_arr[ii]) / sxsiz_arr[ii] ;
     x01_2d[0][ii] = PRED01(x01_2d[0][ii]) ;  /* make sure is in range 0..1 */
   }
   if( scalx != SC_BOX ) xreduce( ndim, x01_2d[0] ) ;
   (void)calfun_(&n,x01_2d[0],x01val+0) ;  /* value of keeper #0 = input vector */
   ntot++ ;                           /* number of times calfun_() is called */
   CALFUN_ERROR(x01_2d[0]) ; if( !isfinite(x01val[0]) ) x01val[0] = BIGVAL ;

   /*-- do a random search for the best starting vector? --*/

   if( nrand > 0 ){
     static double *xtest = NULL ; static int xtest_len = 0 ;
     double *qpar,*cpar , ftest , dist ;
     ...
     /* Step 1: search nrand start vectors, keeping the
                ...
                N.B.: we do NOT displace the input vector in x01_2d[0];
                      therefore, there are nkeep+1 vectors being kept */

     if( verb )
       fprintf(stderr,"++ Powell: random search of %d vectors in %d-dim space\n",nrand,ndim);
       ...
       for( tt=1 ; tt <= nkeep ; tt++ ){          /* is this better than what */
         if( ftest < x01val[tt] ){                    /* we've seen thus far? */
           for( jj=nkeep-1 ; jj >= tt ; jj-- ){    /* push those above #tt up */
             memcpy( x01_2d[jj+1] , x01_2d[jj] , sizeof(double)*ndim ) ; /* in list */
             x01val[jj+1] = x01val[jj] ;
           }
           memcpy( x01_2d[tt] , xtest , sizeof(double)*ndim ) ;  /* save in list */
           x01val[tt] = ftest ;
     ...
     if( verb )
       fprintf(stderr,"++ Powell: 1st round optimization on %d vectors\n",nkeep);

     rb = 0.05 ; re = 0.005 ; mf = 9*ndim+7 ; tbest = 0 ; vbest = BIGVAL ;
     for( tt=0 ; tt < nkeep ; tt++ ){
       (void)newuoa_( &n, &npt, (doublereal *)x01_2d[tt], &rb,&re,&mf,w,&icode ) ;
       for( ii=0 ; ii < ndim ; ii++ ) x01_2d[tt][ii] = PRED01(x01_2d[tt][ii]) ;
       if( scalx != SC_BOX ) xreduce(ndim,x01_2d[tt]) ;
       (void)calfun_(&n,x01_2d[tt],x01val+tt) ; ntot += icode+1 ;
       CALFUN_ERROR(x01_2d[tt]) ; if( !isfinite(x01val[tt]) ) x01val[tt] = BIGVAL ;
       if( x01val[tt] < vbest ){ vbest = x01val[tt]; tbest = tt; }
       if( verb > 1 )
         fprintf(stderr," + %2d: cost = %g %c nfunc=%d\n",tt,x01val[tt],(tbest==tt)?'*':' ',(int)icode) ;
     }

     /* Step 2b: sort results by new x01val costs */

     qsort_doublestuff( nkeep , x01val , (void **)x01_2d ) ;

     /* Step 2c: cast out those that are too close to better vectors
                 in the max-norm (we always keep the best vector in x01_2d[0]) */

#undef  DTHRESH
#define DTHRESH 0.05    /* max-norm distance threshold for vector reject */

     for( tt=1 ; tt < nkeep ; tt++ ){
       qpar = x01_2d[tt] ;                       /* do we keep this vector? */
       for( jj=0 ; jj < tt ; jj++ ){       /* loop over previous keepers */
         if( x01val[jj] >= BIGVAL ) continue ;       /* already rejected */
         cpar = x01_2d[jj] ;                        /* compare qpar to cpar */
     ...
     if( nkeep == 0 ) fprintf(stderr,"** ERROR: Powell: nothing survived 1st round :-(\n") ;
   ...
   /****** fully optimize each of the first ntry-th vectors in x01_2d[] ******/

   if( verb )
     fprintf(stderr,"++ Powell: 2nd round optimization on %d vectors\n",ntry) ;

   tbest = 0 ; vbest = BIGVAL ;
   for( tt=0 ; tt < ntry ; tt++ ){
     (void)newuoa_( &n , &npt , (doublereal *)x01_2d[tt] ,
                    &rhobeg , &rhoend , &maxfun , w , &icode ) ;
     for( ii=0 ; ii < ndim ; ii++ ) x01_2d[tt][ii] = PRED01(x01_2d[tt][ii]) ;
     if( scalx != SC_BOX ) xreduce(ndim,x01_2d[tt]) ;
     (void)calfun_(&n,x01_2d[tt],x01val+tt) ; ntot += icode+1 ;
   ...
   (void)newuoa_( &n , &npt , (doublereal *)x01_2d[tbest] ,
                  &rb , &rhoend , &maxfun , w , &icode ) ;
   for( ii=0 ; ii < ndim ; ii++ ) x01_2d[tbest][ii] = PRED01(x01_2d[tbest][ii]) ;
   if( scalx != SC_BOX ) xreduce(ndim,x01_2d[tbest]) ;
   (void)calfun_(&n,x01_2d[tbest],&vbest) ; ntot += icode+1 ;

   /*-- Rescale bestest output vector back to 'true' range --*/

   for( ii=0 ; ii < ndim ; ii++ )
     x[ii] = sxmin_arr[ii] + x01_2d[tbest][ii] * sxsiz_arr[ii] ;
