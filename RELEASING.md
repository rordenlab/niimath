...
- **`pyproject.toml` build requires stay unconditional and extra-free.** `scikit-build-core[pyproject]` named an extra that scikit-build-core 1.x no longer defines, and pip 25 then failed to backtrack around an unrelated pin. `scikit-build-core>=0.5` is the floor because Linux still builds cp37 wheels, which need 0.10.7.
