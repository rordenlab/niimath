...
- **Readout ramp sign.** `refill_calc_fms.m` forms `angle(exp(1i*(epi_p2-epi_p1)))` with `p2` the REFILL volume, `refill_calc_readout_gradient.m` halves its x-slope, and `refill_calc_fms.m` SUBTRACTS the resulting ramp. The paper's Eq. 7 defines φG_EPI from θ₁ − θ_REFILL, the opposite sign, so the code doubles the residual gradient instead of removing it. Measured on the oracle (in-mask x-slope of EPI minus FLASH field map, rad/s per voxel): sub1 0.785 with the reference sign vs 0.009 with the paper's; sub2 −0.334 vs −0.167; sub3 −0.560 vs −0.347. A synthetic fixture in `release_smoke.py` pins both behaviours. `-ramp-fix` applies the paper's sign.
  End-to-end effect, measured with `validation/rampfix_compare.sh` (both signs through `-refill-epifm`, `-refill-centre`, `-refill-unwarp`; the field-map columns are the 100-volume mean of the unwarped, demedianed EPI maps against the zero-centred FLASH map inside the GE mask; `r` is the in-mask correlation of the corrected series with the FLASH static arm `-fugue`):

  | subject | gradient (Hz/voxel) | slope, reference sign | slope, `-ramp-fix` | RMS ref / fix (rad/s) | r(DDC, SDC) ref / fix |
  | --- | --- | --- | --- | --- | --- |
  | sub1 | −0.001359 | +1.253 | +0.489 | 130.8 / 128.9 | 0.9566 / 0.9620 |
  | sub2 | +0.000293 | +0.151 | +0.315 | 133.2 / 133.3 | 0.9461 / 0.9465 |
  | sub3 | +0.000373 | −0.007 | +0.202 | 124.7 / 124.6 | 0.8577 / 0.8579 |

  Read it as: the sign matters where the measured gradient is large (sub1, five times the others: the residual slope falls by 0.76 rad/s per voxel and agreement with the static arm rises), and is below the noise of the comparison where it is small (sub2/sub3, ±0.2 rad/s per voxel either way against a 125-133 rad/s RMS disagreement between the two arms). The volume-1 slopes quoted above and these 100-volume means differ because the latter are in unwarped space over the whole series. The MATLAB reference now carries the same switch (`data.ramp_fix`, default 0, `refill_set_defaults.m` / `refill_calc_readout_gradient.m`), and with `ramp_fix=1` its sub1 output matches `-ramp-fix` at the ordinary thresholds (field maps max 1.2e-4 rad/s, `epi_m_ddc` max 0.011, median −160.00 both), so the two switches are the same operation.
